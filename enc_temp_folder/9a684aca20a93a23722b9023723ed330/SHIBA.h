...
void initTextures() {
	...
	image = SOIL_load_OGL_texture("assets/textures/tiles/notes_top.png", SOIL_LOAD_RGBA, SOIL_CREATE_NEW_ID, SOIL_FLAG_NTSC_SAFE_RGB);
	...
	*/
	...
	for (int i = 0; i < 4; i++) {
		...
		glPushMatrix();
			...
			switch (i) {
				case 0: glRotatef(0, 0, 1, 0); break;
				case 1: glRotatef(90, 0, 1, 0); break;
				case 2: glRotatef(180, 0, 1, 0); break;
				case 3: glRotatef(270, 0, 1, 0); break;
			}
...
void notesModel(ShibaObject a) {
	...
	glColor3f(0.36, 0.396, 0.196);
		...
		int angle = animateTex(a.color, 1800, true);

		
		glEnable(GL_TEXTURE_2D);
        glBindTexture(GL_TEXTURE_2D, textureCollection.at(Notes));
