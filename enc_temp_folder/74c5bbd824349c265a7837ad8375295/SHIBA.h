...
std::queue<ShibaQuad> aStarImplementation(ShibaObject& entity, Position goal, int range) {
	...
	*/
	...
	//checking there's already a path that's been added.
	...
	while (iterations < 12 && source.cost != 0 && entity.pathing.empty() && goal.toTile().x == cameraPosition.toTile().x && goal.toTile().z == cameraPosition.toTile().z) {
	...
	// if player hasn't moved and there's already a path established finish that first.
	if (!entity.pathing.empty() && goal.toTile().x == cameraPosition.toTile().x && goal.toTile().z == cameraPosition.toTile().z) {
		std::cout << "Finishing existing path first. Trace skipped" << std::endl;
		return entity.pathing;
	}
