	...
	*/
	...
	while (iterations < 15) {
		...
		for (int i = 0; i < 4; i++) {
			...
			// TODO: Add other obstacle checks here
			if (trace.frontObject != -1 
				...
				&&
				levelQueue.front().levelGrid[objectCollection.at(trace.frontObject).tileZ][objectCollection.at(trace.frontObject).tileX] != DoorClosed
				&&
		...
		switch (pathMap.at(lowestCostID).direction) {
			...
			case LEFT: txt = "LEFT"; break;
			case RIGHT: txt = "RIGHT"; break;

		}

		std::cout << "going " << txt << " Cost:" << pathMap.at(lowestCostID).cost << std::endl;
		trace.facing = { 
