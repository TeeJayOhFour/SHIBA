...
std::queue<ShibaQuad> aStarImplementation(ShibaObject& entity, Position& goal, int range) {
	...
	for (int i = 0; i < range; i++) {
		...
		tempPos.z = (entity.getRawCoords().z - range) + i;

		for (int j = 0; j < 2; j++) {	// using 0 and 1 as indicators for north and south
			...
			for (int k = 0; k < range; k++) {
				...
				if (j == 0) {
					tempPos.facing = { true, false, false, false };
					tempPos.x += j + k;
				}
				else {
					tempPos.facing = { false, true , false, false };
					tempPos.x -= j + k;
				}
				
				checkForInteraction(tempPos);
				if (tempPos.frontObject != -1) objectIDRange.push_back(tempPos.frontObject);
	...
	// printing the range
	for (int i = 0; i < objectIDRange.size(); i++) {

		objectCollection.at(i).color = Boss;
		objectCollection.at(i).texture = false;

		for (int j = 0; j < range * 2 + 1; j++) std::cout << " " << i;
