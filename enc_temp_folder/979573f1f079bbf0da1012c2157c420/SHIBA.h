...
void renderGameElements() {
	...
	if (playerInCombat) {
		...
		combatMusic->setVolume(1);

		// if no sound is allocated.
		if (!combatMusic) combatMusic = soundEngine->play2D(BATTLE_MUSIC_0, false, true, true);
		else combatMusic->setIsPaused(!playerInCombat);
		
		if (combatMusic->isFinished() && !combatMusic->isLooped()) 
			combatMusic = soundEngine->play2D(BATTLE_MUSIC_1, true, false, true);

	}
	else {

		combatMusic->setIsLooped(false);

		//start fading music.
		//this doesn't work
		//this still doesn't work

		for (float i = 1.0; i > 0.0; i -= 0.1f) {
			combatMusic->setVolume(i);
		}
		//std::cout << "Current volume" << combatMusic->getVolume() << std::endl;
...
void enemyPathing() {
	...
	if (levelSpawning && enemyCollection.size() < levelQueue.front().enemies) {

		int id = enemySpawnerLocations.at(randomInt(0, enemySpawnerLocations.size() - 1));

		float spawnerX = objectCollection.at(id).tileX;
		float spawnerZ = objectCollection.at(id).tileZ;
		...
		std::vector <Position> emptySpots;
		
		std::cout << "\nADJACENT TILES" << std::endl;
		std::cout << "FORWARD: " << spawnerX + 1 << " : " << spawnerZ << std::endl;
		std::cout << "BACKWARD: " << spawnerX - 1 << " : " << spawnerZ << std::endl;
		std::cout << "LEFT: " << spawnerX << " : " << spawnerZ - 1 << std::endl;
		std::cout << "RIGHT: " << spawnerX << " : " << spawnerZ + 1 << std::endl;


		emptySpots.push_back({ spawnerX, GROUNDLEVEL, spawnerZ + 10 });

		if (emptySpots.size() == 0) {
			std::cout << "Error for spawner in X: " << spawnerX << " Z:" << spawnerZ << " ObjectID: " << id;
			std::cout << "No free space available for enemies to spawn. Skipping this spawner" << std::endl;
		}
		else {
			...
			spawnerX = emptySpots.at(randID).x;
			spawnerZ = emptySpots.at(randID).z;
