...
void enemyPathing() {
	...
	for (auto& item : enemyCollection) {
		...
		if (!item.second.pathing.empty()) {
			...
			item.second.offset += (difference / (ANIMATIONSTEP * 2.2f));

			float randomOffset = 1 / (float) randomInt(0, 10);

			if ((front.toPosition() - (current.toPosition() + item.second.offset)).absolute() <= 0.01f + TILESIZE + randomOffset) {
	...
	std::cout << absoluteBool << std::endl;

	if (absoluteBool > 0) playerInCombat = true;
...
std::queue<ShibaQuad> aStarImplementation(ShibaObject& entity, Position goal, int range) {
	...
	while (iterations < 12 && source.cost != 0 && !samePos && !goal.toTile().motion.isMoving()) {
