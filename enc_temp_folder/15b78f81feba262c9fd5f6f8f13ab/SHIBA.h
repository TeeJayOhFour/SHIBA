...
std::queue<ShibaQuad> aStarImplementation(ShibaObject& entity, Position& goal, int range) {
	...
	std::cout << "Enemy Coords for " << entity.objectName << ": " << entity.tileX << ", " << entity.tileZ << std::endl;
	std::cout << "Standing on " << tempPos.frontObject << std::endl;
	
	objectIDRange.push_back(objectIDRange.at(tempPos.frontObject));

	// for every pole.
	for (int i = 0; i < 4; i++) {

		tempPos.facing = { i == 0, i == 1, i == 2, i == 3 };
