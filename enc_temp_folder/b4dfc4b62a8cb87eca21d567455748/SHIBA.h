...
std::queue<ShibaQuad> aStarImplementation(ShibaObject& entity, Position& goal, int range) {
	...
	for (int r = 0; r < range * 2 + 1; r++) {
		...
		for (int i = 0; i < 2; i++) {
			...
			for (int j = 0; j < range; j++) {
				...
				tempPos.x = i == 0 ? tempPos.x + (j * 10) : tempPos.x - (j * 10);
	...
	for (int i : objectIDRange) {
		...
		// adding a green plane to show range.
		// turns red when a player steps within it.
		;


		glBegin(GL_QUADS);
		getColorMod(Boss);
			glNormal3f(objectCollection.at(i).vertexCol.at(0).normal[0], objectCollection.at(i).vertexCol.at(0).normal[1], objectCollection.at(i).vertexCol.at(0).normal[2]);

			glVertex3f(objectCollection.at(i).vertexCol.at(0).x, objectCollection.at(i).vertexCol.at(0).y, objectCollection.at(i).vertexCol.at(0).z);
			glVertex3f(objectCollection.at(i).vertexCol.at(1).x, objectCollection.at(i).vertexCol.at(1).y, objectCollection.at(i).vertexCol.at(1).z);
			glVertex3f(objectCollection.at(i).vertexCol.at(2).x, objectCollection.at(i).vertexCol.at(2).y, objectCollection.at(i).vertexCol.at(2).z);
			glVertex3f(objectCollection.at(i).vertexCol.at(3).x, objectCollection.at(i).vertexCol.at(3).y, objectCollection.at(i).vertexCol.at(3).z);

		glEnd();

		//objectCollection.at(i).color = Boss;
		//objectCollection.at(i).texture = false;
