...
void enemyPathing() {
	...
	if (levelSpawning && enemyCollection.size() < levelQueue.front().enemies) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (objectCollection.at(tempEnemy.frontObject).color == Empty) emptySpots.push_back(tempEnemy.frontObject);
		...
		if (emptySpots.size() == 0) {
			...
			spawnerZ = objectCollection.at(emptySpots.at(randID)).tileX * 10;
