...
void enemyPathing() {
	...
	if (levelSpawning && enemyCollection.size() < levelQueue.front().enemies) {
		...
		for (int i = 0; i < 4; i++) {
			...
			std::cout << "TILE ID: " << tempEnemy.frontObject << std::endl;


			if (tempEnemy.frontObject != -1 && 
				objectCollection.at(tempEnemy.frontObject).color == Empty) 
		...
		if (emptySpots.size() == 0) {
			...
			spawnerX = objectCollection.at(emptySpots.at(randID)).tileX;
			spawnerZ = objectCollection.at(emptySpots.at(randID)).tileZ;
