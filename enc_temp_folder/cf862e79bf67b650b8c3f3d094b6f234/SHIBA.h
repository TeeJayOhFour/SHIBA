...
void enemyPathing() {
	...
	for (auto& item : enemyCollection) {
		...
		if (!item.second.pathing.empty()) {
			...
			if (difference.z < 0) item.second.offset.facing.south;
			else if (difference.z > 0) item.second.offset.facing.north = true;
			else if (difference.x < 0) item.second.offset.facing.east = true;
			else if (difference.x > 0) item.second.offset.facing.west = true;
			...
			if ((front.toPosition() - (current.toPosition() + item.second.offset)).absolute() <= 0.01f) {
				...
				item.second.offset.facing.printFacing();
