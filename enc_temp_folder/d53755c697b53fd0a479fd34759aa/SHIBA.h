...
void initTextures() {
	...
	image = SOIL_load_OGL_texture("assets/textures/ceiling.png", SOIL_LOAD_RGBA, SOIL_CREATE_NEW_ID, SOIL_FLAG_NTSC_SAFE_RGB);
	...
	image = SOIL_load_OGL_texture("assets/textures/door.png", SOIL_LOAD_RGBA, SOIL_CREATE_NEW_ID, SOIL_FLAG_NTSC_SAFE_RGB);
	textureCollection.insert_or_assign(DoorClosed, image);

	image = SOIL_load_OGL_texture("assets/textures/door2.png", SOIL_LOAD_RGBA, SOIL_CREATE_NEW_ID, SOIL_FLAG_NTSC_SAFE_RGB);
	textureCollection.insert_or_assign(LevelExit, image);

	image = SOIL_load_OGL_texture("assets/textures/door3.png", SOIL_LOAD_RGBA, SOIL_CREATE_NEW_ID, SOIL_FLAG_NTSC_SAFE_RGB);
...
static void updateHUD() {
	...
	if (player.health >= 70) glColor3f(0, .5, 0);
	else if (player.health >= 30) glColor3f(.5, .5, 0);
	else glColor3f(.5, 0, 0);
	...
	glBegin(GL_QUADS);
		...
		glVertex2f(369 * xMod, 629);

		glVertex2f(369 * xMod, 650);
...
void draw() {
	...
	for (int i = 0; i < objectCollection.size(); i++) {
		...
		if (objectCollection.at(i).glutSolids != nullptr && abs(objectCollection.at(i).color) != Custom) {
...
void renderWorldBox() {
	...
	*/
	...
	glPushMatrix();
		...
		glRotatef(90, 0, 1, 0);	//rotate the object with respect to camera yaw
		...
		glNormal3f(0, -1, 0);
			...
			glTexCoord2f(0, 1.0);
			glVertex3f(0.2, 0, -650.0f);

			glTexCoord2f(1, 1.0);
			glVertex3f(0, 0, 650.0f);

			glTexCoord2f(1, 0.0);
			glVertex3f(0, 400, 650.0f);
	...
	for (int actualX = 0; actualX < size; actualX++) {
		...
		for (int actualZ = size - 1; actualZ > -1; actualZ--) {
			...
			if (tile.color != Custom || levelQueue.front().customObjects.empty()) {
				...
				if (tile.color == Objective) tile.setLoadGlutFunction(eggModel);
	...
	for (int r = 0; r < range * 2 + 1; r++) {
		...
		for (int i = 0; i < 2; i++) {
			...
			if (tempPos.frontObject != -1 && objectCollection.at(tempPos.frontObject).color != Wall &&
				...
				&& objectCollection.at(tempPos.frontObject).color != SPAWNER_1 && objectCollection.at(tempPos.frontObject).color != SPAWNER_2
				&& objectCollection.at(tempPos.frontObject).color != SPAWNER_3
