...
void initTextures() {
	...
	GLuint door = SOIL_load_OGL_texture("assets/textures/door.bmp", SOIL_LOAD_RGBA, SOIL_CREATE_NEW_ID, SOIL_FLAG_NTSC_SAFE_RGB);
	textureCollection.insert_or_assign(DoorClosed, door);

	GLuint enemy = SOIL_load_OGL_texture("assets/textures/enemy1.png", SOIL_LOAD_RGBA, SOIL_CREATE_NEW_ID, SOIL_FLAG_NTSC_SAFE_RGB);
	textureCollection.insert_or_assign(Enemy, enemy);

	//image = loadTexture("assets/textures/door.bmp");
	//parseTexture(image, DoorClosed);

	GLuint yourmom = SOIL_load_OGL_texture("assets/textures/sp.png", SOIL_LOAD_RGBA, SOIL_CREATE_NEW_ID, SOIL_FLAG_NTSC_SAFE_RGB);


	textureCollection.insert_or_assign(SplashArt0, yourmom);
...
void listenForMouseClick(int button, int state, int x, int y) {
	...
	case 0:
		...
		if (state && track) {
			std::string posID = std::to_string(cameraPosition.toTile().x) + std::to_string(cameraPosition.toTile().z);
			
			// checking the enemy is in the same tile as the player, in which case melee them to one shot.
			if (enemyCollection.count(posID) == 0)
				melee(posID);
			else shoot(cameraPosition);
		}
...
void shoot(Position entity) {
	...
	bullet.objectName = std::to_string(bullet.vertexCol.at(0).toPosition().toTile().x) + std::to_string(bullet.vertexCol.at(0).toPosition().toTile().z);
...
void enemyPathing() {
	...
	if (levelSpawning && enemyCollection.size() < levelQueue.front().enemies/2) {
		...
		if (emptySpots.size() == 0) {
			...
			tempEnemy.objectName = std::to_string(tempEnemy.vertexCol.at(0).toPosition().toTile().x) + std::to_string(tempEnemy.vertexCol.at(0).toPosition().toTile().z);
...
void enemyModel(ShibaObject a) {
	...
	glPushMatrix();
		...
		glBindTexture(GL_TEXTURE_2D, textureCollection.at(Enemy));
