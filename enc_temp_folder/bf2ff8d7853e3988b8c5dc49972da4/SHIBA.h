...
void draw() {
	...
	for (int i = 0; i < objectCollection.size(); i++) {
		...
		if ((objectCollection.at(i).color == Wall_3) || objectCollection.at(i).glutSolids != nullptr 
			&& abs(objectCollection.at(i).color) != Custom) {
...
void renderWorldBox() {
	...
	for (int xCounter = 0; xCounter < size; xCounter++) {
		...
		for (int zCounter = 0; zCounter < size; zCounter++) {
			...
			} else glBindTexture(GL_TEXTURE_2D, textureCollection.at(Empty));
...
void initLevels(std::queue <Level> queue) {
	...
	std::cout << "Initializing Level " << levelQueue.front().name << "..." << std::endl;
		...
		for (int actualZ = size - 1; actualZ > -1; actualZ--) {
			...
			if (tile.color != Custom || levelQueue.front().customObjects.empty()) {
				...
				if (tile.color == Wall) {
					...
					switch (texID) {
						case 0: tile.color = Wall; break;
						case 1: tile.color = Wall_1; break;
						case 2: tile.color = Wall_2; break;
						case 3: tile.color = Wall_3; break;
					};
