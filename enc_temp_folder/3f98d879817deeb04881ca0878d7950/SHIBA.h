	...
	for (int r = 0; r < range * 2 + 1; r++) {
		...
		for (int i = 0; i < 2; i++) {
			...
			for (int j = 0; j < range; j++) {
				...
				tempPos.x = i == 0 ? tempPos.x + (10 * j) : tempPos.x - (10 * j);
...
void enemyModel(ShibaObject a) {
	...
	if (!a.rangeIDCol.empty()) {
		...
		for (int i : a.rangeIDCol) {
			...
			if (objectCollection.at(i).tileX == cameraPosition.toTile().x && objectCollection.at(i).tileZ == cameraPosition.toTile().z) glColor3f(1, 0, 0);
			else {
				glColor3f(0, 1, 0);
			}
