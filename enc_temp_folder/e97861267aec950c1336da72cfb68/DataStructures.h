...
public:
	...
	void render(int x, int y, int padding, int textHeight, Position* cursor) {
		...
		case SLIDER_BUTTON:
			//override hover area.
			if (cursor->x > x && cursor->x < x + (this->text.length() * 13)
				&& cursor->y > y - padding - textHeight && cursor->y < y - textHeight) {
			...
			//Draw slider box
			glVertex2f(x, y - padding - textHeight - 10);
			glVertex2f(x + 15, y - padding - textHeight - 10);
			glVertex2f(x + 15, y - textHeight - 10);
			glVertex2f(x, y - textHeight - 10);

				
			//Draws slider line
			glVertex2f(x, y - padding - textHeight);
			glVertex2f(x + (text.length() * 13), y - padding - textHeight);
	...
	void show() {
		...
		if (displayed) {
			...
			for (MenuOption &item : options) {
				...
				if (item.hovering) {
					...
					if (cursor->buttonState == 0 && cursor->button == 0) {
						...
						if (item.type == BUTTON)
							...
							handlerFunction(item.text, item.value.at(0));
						}
				...
				if (item.type == SLIDER_BUTTON) y += 60.0f;
				else y += 30.0f;
			}
